...
public class IexecHubService extends IexecHubAbstractService {
	...
	/**
	 * Check if a deal's task can be initialized. An initializable task should have
	 * the status UNSET onchain and have a contribution deadline that is still in
	 * the future.
	 * 
	 * @param chainDealId
	 * @param taskIndex
	...
	/**
	 * Check if the task is defined onchain and has the status
	 * {@link ChainTaskStatus#UNSET}.
	 * 
	 * @param chainDealId
	 * @param taskIndex
	...
	public boolean isTaskInUnsetStatusOnChain(String chainDealId, int taskIndex) {
		...
		return chainTask.isEmpty() || ChainTaskStatus.UNSET.equals(chainTask.get().getStatus());
	}

	/**
	 * Check if a deal's contribution deadline is still not reached.
	 * 
	 * @param chainDealId
	 * @return true if deadline is not reached, false otherwise.
	 */
	public boolean isBeforeContributionDeadline(String chainDealId) {
		return getChainDeal(chainDealId).map(this::isBeforeContributionDeadline).orElse(false);
	}
	...
	public boolean isBeforeContributionDeadline(ChainDeal chainDeal) {
		return getChainDealContributionDeadline(chainDeal).after(new Date());
	}

	/**
	 * <p>
	 * Get deal's contribution deadline date. The deadline is calculated as follow:
	 ...
	 * <li>nbOfCategoryUnits: number of category units dedicated for the
	 * contribution phase.
	 * 
	 * @param chainDeal
	 * @return
	...
	/**
	 * <p>
	 * Get deal's final deadline date. The deadline is calculated as follow: start +
	 * maxCategoryTime * 10.
	 * 
	 * <ul>
	 * <li>start: the start time of the deal.
	 * <li>maxCategoryTime: duration of the deal's category.
	 * <li>10: number of category units dedicated for the hole execution.
	 * 
	 * @param chainDeal
	 * @return
	 */
	...
	public Optional<Pair<String, ChainReceipt>> initialize(String chainDealId, int taskIndex) {
		log.info("Requested  initialize [chainDealId:{}, taskIndex:{}, waitingTxCount:{}]", chainDealId, taskIndex,
				getWaitingTransactionCount());
		try {
			return CompletableFuture.supplyAsync(() -> sendInitializeTransaction(chainDealId, taskIndex), executor)
					.get();
	...
	private Optional<Pair<String, ChainReceipt>> sendInitializeTransaction(String chainDealId, int taskIndex) {
		...
		try {
			...
			log.error("Failed to send initialize [chainDealId:{}, taskIndex:{}, exception:{}]", chainDealId, taskIndex,
					e.getMessage());
			e.printStackTrace();
			return Optional.empty();
		}

		List<IexecHubContract.TaskInitializeEventResponse> initializeEvents = getHubContract()
				.getTaskInitializeEvents(initializeReceipt);
	...
	public boolean canFinalize(String chainTaskId) {
		...
		if (ret) {
			...
			log.warn(
					"Can't finalize [chainTaskId:{}, "
							+ "isChainTaskStatusRevealing:{}, isFinalDeadlineInFuture:{}, hasEnoughRevealors:{}]",
					chainTaskId, isChainTaskStatusRevealing, isFinalDeadlineInFuture, hasEnoughRevealors);
		}
		return ret;
	}
	...
	public Optional<ChainReceipt> finalizeTask(String chainTaskId, String resultLink, String callbackData) {
		...
		try {
			return CompletableFuture
					.supplyAsync(() -> sendFinalizeTransaction(chainTaskId, resultLink, callbackData), executor).get();
		} catch (InterruptedException | ExecutionException e) {
			e.printStackTrace();
		}
		return Optional.empty();
	...
	private Optional<ChainReceipt> sendFinalizeTransaction(String chainTaskId, String resultLink, String callbackData) {
		...
		try {
			...
			log.error(
					"Failed to send finalize [chainTaskId:{}, resultLink:{}, callbackData:{}, shouldSendCallback:{}, error:{}]]",
					chainTaskId, resultLink, callbackData, shouldSendCallback, e.getMessage());
			return Optional.empty();
		}

		List<IexecHubContract.TaskFinalizeEventResponse> finalizeEvents = getHubContract()
				.getTaskFinalizeEvents(finalizeReceipt);
		...
		if (isSuccessTx(chainTaskId, finalizeEvent, COMPLETED)) {
			...
			log.info(
					"Finalized [chainTaskId:{}, resultLink:{}, callbackData:{}, "
							+ "shouldSendCallback:{}, gasUsed:{}, block:{}]",
					chainTaskId, resultLink, callbackData, shouldSendCallback, finalizeReceipt.getGasUsed(),
					chainReceipt.getBlockNumber());
	...
	public boolean canReopen(String chainTaskId) {
		...
		if (check) {
			...
			log.warn(
					"Can't reopen [chainTaskId:{}, " + "isChainTaskStatusRevealing:{}, isBeforeFinalDeadline:{}, "
							+ "isAfterRevealDeadline:{}, revealCounterEqualsZero:{}]",
					chainTaskId, isChainTaskStatusRevealing, isBeforeFinalDeadline, isAfterRevealDeadline,
					revealCounterEqualsZero);
		}
		return check;
	}
	...
	private Optional<ChainReceipt> sendReopenTransaction(String chainTaskId) {
		...
		try {
			receipt = getHubContract(web3jService.getWritingContractGasProvider()).reopen(stringToBytes(chainTaskId))
					.send();
		} catch (Exception e) {
		...
		log.info("Reopened [chainTaskId:{}, gasUsed:{}, block:{}]", chainTaskId, receipt.getGasUsed(),
				chainReceipt.getBlockNumber());
		return Optional.of(chainReceipt);
	}
	...
	private ChainReceipt buildChainReceipt(TransactionReceipt receipt) {
		return ChainReceipt.builder().txHash(receipt.getTransactionHash())
				.blockNumber(receipt.getBlockNumber() != null ? receipt.getBlockNumber().longValue() : 0).build();
	}

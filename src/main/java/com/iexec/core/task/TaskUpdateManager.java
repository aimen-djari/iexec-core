...
public class TaskUpdateManager implements TaskUpdateRequestConsumer {
	...
	public TaskUpdateManager(TaskService taskService, TaskRepository taskRepository,
			TaskUpdateRequestManager taskUpdateRequestManager, IexecHubService iexecHubService,
			ReplicatesService replicatesService, ApplicationEventPublisher applicationEventPublisher,
	...
	void updateTask(String chainTaskId) {
		...
		boolean isFinalDeadlinePossible = !TaskStatus.getStatusesWhereFinalDeadlineIsImpossible()
				.contains(currentStatus);
		boolean isServiceTask = task.isService();
		if (isFinalDeadlinePossible && new Date().after(task.getFinalDeadline()) && !isServiceTask) {
	...
	private void toInterrupted(Task task) {
		log.info("TaskUpdateManager toInterrupted() triggered");

		blockchainAdapterService.isFinalized(task.getChainTaskId()).ifPresentOrElse(isSuccess -> {
			if (isSuccess != null && isSuccess) {
				log.info("Finalized on blockchain (tx mined)" + "[chainTaskId:{}]", task.getChainTaskId());
				updateTaskStatusAndSave(task, INTERRUPTED, null);

				applicationEventPublisher
						.publishEvent(PleaseAbortEvent.builder().chainTaskId(task.getChainTaskId()).build());
				return;
			}
		...
		}, () -> log.error("Unable to check finalization on blockchain " + "(likely too long), should use a detector "
	...
	Task updateTaskStatusAndSave(Task task, TaskStatus newStatus, ChainReceipt chainReceipt) {
		...
		log.info("UpdateTaskStatus suceeded [chainTaskId:{}, currentStatus:{}, newStatus:{}]", task.getChainTaskId(),
	...
	void received2Initializing(Task task) {
		...
		if (!isCurrentStatusReceived) {
			log.error("Cannot initialize task [chainTaskId:{}, currentStatus:{}]", task.getChainTaskId(),
					task.getCurrentStatus());
			return;
		}

		boolean hasEnoughGas = iexecHubService.hasEnoughGas();
		boolean isTaskUnsetOnChain = iexecHubService.isTaskInUnsetStatusOnChain(task.getChainDealId(),
				task.getTaskIndex());
		...
		if (!hasEnoughGas || !isTaskUnsetOnChain || !isBeforeContributionDeadline) {
			log.error(
					"Cannot initialize task [chainTaskId:{}, hasEnoughGas:{}, "
		...
		blockchainAdapterService.requestInitialize(task.getChainDealId(), task.getTaskIndex())
				.filter(chainTaskId -> chainTaskId.equalsIgnoreCase(task.getChainTaskId()))
				.ifPresentOrElse(chainTaskId -> {
					log.info("Requested initialize on blockchain [chainTaskId:{}]", task.getChainTaskId());
					updateTaskStatusAndSave(task, INITIALIZING);
					// Watch initializing to initialized
					updateTask(task.getChainTaskId());
				}, () -> {
					log.error("Failed to request initialize on blockchain [chainTaskId:{}]", task.getChainTaskId());
	...
	void initializing2Initialized(Task task) {
		...
		blockchainAdapterService.isInitialized(task.getChainTaskId()).ifPresentOrElse(isSuccess -> {
			if (isSuccess != null && isSuccess) {
			...
			updateTaskStatusAndSave(task, FAILED);
				...
				+ "(likely too long), should use a detector [chainTaskId:{}]", task.getChainTaskId()));
	}
	...
	void initialized2Running(Task task) {
		...
		final ReplicateStatus[] acceptableStatus = new ReplicateStatus[] { ReplicateStatus.STARTED,
				ReplicateStatus.APP_DOWNLOADING, ReplicateStatus.APP_DOWNLOAD_FAILED, ReplicateStatus.APP_DOWNLOADED,
				ReplicateStatus.DATA_DOWNLOADING, ReplicateStatus.DATA_DOWNLOAD_FAILED, ReplicateStatus.DATA_DOWNLOADED,
				ReplicateStatus.COMPUTING, ReplicateStatus.COMPUTE_FAILED, ReplicateStatus.COMPUTED,
				ReplicateStatus.CONTRIBUTING, ReplicateStatus.CONTRIBUTE_FAILED, ReplicateStatus.CONTRIBUTED };
		final int nbReplicatesContainingStartingStatus = replicatesService
				.getNbReplicatesWithLastRelevantStatus(chainTaskId, acceptableStatus);
	...
	public boolean isConsensusReached(Task task) {
		...
		int offChainWinners = isChainTaskRevealing
				? replicatesService.getNbValidContributedWinners(task.getChainTaskId(), chainTask.getConsensusValue())
				: 0;
	...
	void running2ConsensusReached(Task task) {
		...
		if (isTaskInRunningStatus && isConsensusReached) {
			...
			// change the the revealDeadline and consensus of the task from the chainTask
			// info
			task.setRevealDeadline(new Date(chainTask.getRevealDeadline()));
			task.setConsensus(chainTask.getConsensusValue());
			long consensusBlockNumber = iexecHubService
					.getConsensusBlock(task.getChainTaskId(), task.getInitializationBlockNumber()).getBlockNumber();
			task.setConsensusReachedBlockNumber(consensusBlockNumber);
			updateTaskStatusAndSave(task, CONSENSUS_REACHED);

			applicationEventPublisher.publishEvent(ConsensusReachedEvent.builder().chainTaskId(task.getChainTaskId())
					.consensus(task.getConsensus()).blockNumber(task.getConsensusReachedBlockNumber()).build());
		}
	}

	void toInterrupting(Task task) {
		boolean isInterruptionPossible = !TaskStatus.getStatusesWhereInterruptionIsImpossible().contains(task.getCurrentStatus());
		boolean isInterrupted = task.isInterrupted();

		if (isInterruptionPossible && isInterrupted) {
			log.info("Task Update Manager toInterrupting() triggered");
			Optional<ChainTask> optional = iexecHubService.getChainTask(task.getChainTaskId());
			if (optional.isEmpty())
				return;
			ChainTask chainTask = optional.get();
			...
			blockchainAdapterService
					.requestFinalize(task.getChainTaskId(), task.getResultLink(), task.getChainCallbackData())
					.ifPresentOrElse(chainTaskId -> {
						log.info("Requested finalize on blockchain [chainTaskId:{}]", task.getChainTaskId());
						updateTaskStatusAndSave(task, INTERRUPTING);
						// Watch finalizing to finalized
						updateTask(task.getChainTaskId());
					}, () -> {
						log.error("Failed to request finalize on blockchain [chainTaskId:{}]", task.getChainTaskId());
						updateTaskStatusAndSave(task, FINALIZE_FAILED);
						updateTaskStatusAndSave(task, FAILED);
					});
		}
	...
	void initializedOrRunning2ContributionTimeout(Task task) {
		...
		boolean isNowAfterContributionDeadline = task.getContributionDeadline() != null
				&& new Date().after(task.getContributionDeadline());

		if (isInitializedOrRunningTask && isNowAfterContributionDeadline) {
			if (task.isService()) {
				log.info("Task Update Manager toContributionTimeout() triggered for a service");
				blockchainAdapterService
						.requestFinalize(task.getChainTaskId(), task.getResultLink(), task.getChainCallbackData())
						.ifPresentOrElse(chainTaskId -> {
							log.info("Requested finalize on blockchain [chainTaskId:{}]", task.getChainTaskId());
				...
				updateTaskStatusAndSave(task, FAILED);
				applicationEventPublisher
						.publishEvent(ContributionTimeoutEvent.builder().chainTaskId(task.getChainTaskId()).build());
			}
		}
	}

	/**
	 * In TEE mode, when all workers have failed to run the task, we send the new
	 * status as soon as possible. It avoids a state within which we know the task
	 * won't be updated anymore, but we wait for a timeout. <br>
	 * We consider that all workers are in a `RUNNING_FAILED` status when all alive
	 * workers have tried to run the task, but they have failed.
	 *
	 * @param task Task to check and to make become
	 *             {@link TaskStatus#RUNNING_FAILED}.
	 */
	void running2RunningFailed(Task task) {
		...
		final List<Replicate> replicatesOfAliveWorkers = aliveWorkers.stream().map(Worker::getWalletAddress)
				.map(replicatesList::getReplicateOfWorker).filter(Optional::isPresent).map(Optional::get)
				.collect(Collectors.toList());

		// If at least an alive worker has not run the task, it is not a
		// `RUNNING_FAILURE`.
		...
		// If not all alive workers have failed while running the task, that's not a
		// running failure.
		boolean notAllReplicatesFailed = replicatesOfAliveWorkers.stream().map(Replicate::getLastRelevantStatus)
				.map(Optional::get).anyMatch(Predicate.not(ReplicateStatus::isFailedBeforeComputed));

		if (notAllReplicatesFailed) {
			return;
		}

		// If all alive workers have failed on this task, its computation should be
		// stopped.
		...
		updateTaskStatusAndSave(task, FAILED);
		applicationEventPublisher
				.publishEvent(TaskRunningFailedEvent.builder().chainTaskId(task.getChainTaskId()).build());
	}
	...
	void consensusReached2AtLeastOneReveal2ResultUploading(Task task) {
		...
		boolean condition2 = replicatesService.getNbReplicatesWithCurrentStatus(task.getChainTaskId(),
				ReplicateStatus.REVEALED) > 0;
	...
	void consensusReached2Reopening(Task task) {
		...
		boolean hasAtLeastOneReveal = replicatesService.getNbReplicatesWithCurrentStatus(task.getChainTaskId(),
				ReplicateStatus.REVEALED) > 0;
	...
	void resultUploading2Uploaded(Task task) {
		...
		Optional<Replicate> oUploadedReplicate = replicatesService
				.getReplicateWithResultUploadedStatus(task.getChainTaskId());
		...
		Optional<Replicate> oReplicate = replicatesService.getReplicate(task.getChainTaskId(),
				uploadingReplicateAddress);
	...
	void resultUploading2UploadTimeout(Task task) {
		...
		if (isTaskInResultUploading && isNowAfterFinalDeadline) {
			...
			applicationEventPublisher
					.publishEvent(ResultUploadTimeoutEvent.builder().chainTaskId(task.getChainTaskId()).build());
			updateTaskStatusAndSave(task, FAILED);
		}
	}

	void requestUpload(Task task) {
		boolean isThereAWorkerUploading = replicatesService.getNbReplicatesWithCurrentStatus(task.getChainTaskId(),
				ReplicateStatus.RESULT_UPLOADING, ReplicateStatus.RESULT_UPLOAD_REQUESTED) > 0;

		if (isThereAWorkerUploading) {
			log.info("Upload is requested but an upload is already in process. [chainTaskId: {}]",
					task.getChainTaskId());
			return;
		}

		Optional<Replicate> optionalReplicate = replicatesService
				.getRandomReplicateWithRevealStatus(task.getChainTaskId());
		if (optionalReplicate.isPresent()) {
			...
			applicationEventPublisher
					.publishEvent(new PleaseUploadEvent(task.getChainTaskId(), replicate.getWalletAddress()));
		}
	}
	...
	void resultUploaded2Finalizing(Task task) {
		...
		int offChainReveal = replicatesService.getNbReplicatesContainingStatus(task.getChainTaskId(),
